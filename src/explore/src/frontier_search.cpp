...
FrontierSearch::searchFrom(geometry_msgs::msg::Point position)
{
  ...
  if (nearestCell(clear, pos, FREE_SPACE, *costmap_)) {  //找到最近的空闲单元格
    bfs.push(clear);
  } else {
    bfs.push(pos); //没有找到，直接从当前位置开始搜索
  ...
  while (!bfs.empty()) {
    ...
    for (unsigned int nbr : nhood8(idx, *costmap_)) {
      ...
      if (isNewFrontierCell(nbr, frontier_flag)) {
        ...
        unsigned int mx, my;
        double wx, wy;
        costmap_->indexToCells(nbr, mx, my); // 将地图索引转换为地图坐标
        costmap_->mapToWorld(mx, my, wx, wy); // 将地图坐标转换为世界坐标
